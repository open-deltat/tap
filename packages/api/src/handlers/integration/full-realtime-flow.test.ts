...
import { websocketHandler, WebSocketData } from '../ws/index';
import { createTestTenant, createTestResource, createTestOffer } from '../test-setup';
import type { TenantId, ResourceId, SessionId, Tenant, Resource } from '@tap/core';
    ...
    test('3. Client A places hold -> Client B receives delta', async () => {
        ...
        const waitForDelta = async () => {
            ...
            while (Date.now() - start < 3000) {
                const delta = wsB.sent.find(m => m.includes('"type":"delta"') && m.includes(holdId));
    ...
    test('4. Client A releases hold -> Client B receives delta', async () => {
        ...
        const waitForDelta = async () => {
            ...
            while (Date.now() - start < 3000) {
                ...
                const delta = wsB.sent.find(m => m.includes('"type":"delta"') && m.includes('HoldReleased') && m.includes(holdId));
                if (delta) return delta;
                await new Promise(r => setTimeout(r, 100));
            }
            return null;
