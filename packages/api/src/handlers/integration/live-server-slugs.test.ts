...
import type { Resource, Tenant } from '@tap/core';
...
beforeAll(async () => {
	...
	await new Promise<void>((resolve, reject) => {
		...
		async function readStream(stream: ReadableStream) {
			...
			while (true) {
				...
				const match = buffer.match(
					/TAP API running at http:\/\/localhost:(\d+)/,
				);
				if (match && !foundPort) {
					foundPort = true;
		...
		if (serverProcess.stdout) {
			readStream(serverProcess.stdout).catch((e) =>
...
describe('Live Server Granular Diagnosis', () => {
	...
	test('5. Place Hold via WS', async () => {
		...
		for (let i = 0; i < 20; i++) {
			if (
				messages.find(
					(m) => m.type === 'hold.confirmed' && m.requestId === reqId,
				)
			) {
				confirmed = true;
				break;
