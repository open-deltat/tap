...
export const createResourceRepository = (
	db: SqliteDatabase,
): ResourceRepository => ({
...
export const createOfferRepository = (db: SqliteDatabase): OfferRepository => ({
...
export const createHoldRepository = (db: SqliteDatabase): HoldRepository => ({
	getById: async (id) => {
		const [row] = await db
			.select()
			.from(holds)
...
const AUTH_SCOPES = ['read', 'hold', 'book', 'cancel', 'manage'] as const;
type ValidAuthScope = (typeof AUTH_SCOPES)[number];

const isAuthScope = (value: string): value is ValidAuthScope => {
...
const toApiKey = (row: typeof apiKeys.$inferSelect): ApiKey => {
	...
	const scopes: AuthScope[] = Array.isArray(rawScopes)
		? rawScopes.filter(
				(s): s is AuthScope => typeof s === 'string' && isAuthScope(s),
			)
		: [];
